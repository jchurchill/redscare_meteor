_.mixin({
	...
	groupByAndMap: function(list, keySelector, valueSelector) {
		...
		return _.foldl(list, function(result, value, key) {
			var newKey = keySelectorFunc(value, key);
			(result[newKey] || (result[newKey] = [])).push(valueSelectorFunc(value, key));
			return result;
		}, {});
	},
	...
	indexByAndMap: function(list, keySelector, valueSelector) {
		...
		return _.foldl(list, function(result, value, key) {
			var newKey = keySelectorFunc(value, key);
			result[newKey] = valueSelectorFunc(value, key);
			return result;
		}, {});
	},
	...
	mapInto: function(list, object) {
		return _.map(list, function(v) { return object[v]; });
	},
	...
	mapProperties: function(obj, valueMapFn, context) {
		...
		return _.foldl(obj, function(result, value, key) {
			result[key] = valueSelectorFunc(value, key, obj);
			return result;
		}, {});
	},
	...
	// A version of "pairs" that allows for more readable code. Probably slower though.
	namedPairs: function (obj, keyName, valueName) {
		keyName = keyName || "key";
		valueName = valueName || "value";
		if (keyName === valueName) { throw "key/value cannot have same name"; }
		...
		for (var i = 0; i < length; i++) {
			namedPair = {};
			namedPair[keyName] = keys[i];
			namedPair[valueName] = obj[keys[i]];
			pairs[i] = namedPair;
		}
		return pairs;
	}
